...
| Inline terminal images | Partial | The renderer and app event path can substitute prepared fixed-width cells for visible avatar, badge, emote, and emoji rows; standard emoji provider metadata is available, while default live resolver wiring and manual Kitty/Ghostty validation remain planned. |
