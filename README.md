...
| Theming | Ready | 13 built-in presets (Claude, Codex, Btop, Nord, Dracula, Gruvbox, Solarized Dark, Monokai, One Dark, Tokyo Night, Catppuccin Mocha, Rose Pine, Mono) plus a custom hex palette apply across every widget, including the full terminal background. Theme-derived gradients animate the top bar and splash without introducing hard-coded colors. Chat messages use alternating surfaces, colored delimiter rails, and a mail glyph for stronger visual grouping. `ctrl+t` opens a btop-style settings view that live-previews a theme as you move the selection and persists it with `enter` (`esc` reverts); `twi profile list\|show\|set` manages the same setting from the CLI. |
| Animation | Ready | A shared ~10fps clock (disabled when `animation_mode = "off"`) drives gradient motion, pulsing LIVE/REC and incoming-message rails, a channel-switch flash, typewriter chat and command-palette reveals, and a staged ~2s block-logo startup sequence (skippable by any keypress). Reduced mode slows decorative gradient motion. |
