...
UX, redacted debug logging, and release binary/container packaging
are partial or current for their documented paths; on supported Unix
...
Twitch release validation and exact Docker CLI validation remain
environment-dependent. Terminal-image rendering (Kitty graphics) was
implemented and later removed as a deliberate product decision; avatars,
badges, emotes, and emoji now always render as text, so image-specific risks
below are resolved by removal rather than by manual validation.
...
| RR-003 | Terminal image protocol behavior varies across Kitty, Ghostty, and non-Kitty terminals. | Resolved (removed) | N/A | The Kitty-graphics image renderer was removed entirely; avatars, badges, emotes, and emoji always render as text with no terminal-graphics dependency, so this risk no longer applies. | Asset/image engineer | `twi doctor` no longer reports image/Kitty signals; golden fallback snapshots cover the permanent text rendering. |
| RR-004 | Image cache or asset downloads slow chat rendering. | Resolved (removed) | N/A | The image download/cache/render pipeline was removed entirely; avatar/badge/emote rendering is synchronous text construction with no image I/O to block on. | Asset/image engineer | Code search confirms no image decode/download/cache path remains in `internal/render`. |
...
| RR-013 | Twitch Helix/API outages or quota issues break emote autocomplete or token validation. | Medium | Medium | Treat emote autocomplete and token validation as optional/degradable; keep chat read/send usable without them. Avatars, badges, and Twitch emote tokens no longer depend on Helix at all — they always render as text from local message data. | Twitch integration engineer | Fake API failure tests; fallback golden snapshots; manual degraded-mode status. |
...
5. Stable text-only avatar/badge/emote/emoji fallback contract.
