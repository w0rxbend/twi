...
- Credentials from flags, environment, or config file.
...
Later scope includes multi-channel UX, image-backed avatars/emotes/emoji, richer diagnostics, interactive login, setup wizard, command palette, activity charts, and additional transport/image protocols.
...
| R-002 | Chat send | Send composer text to the active channel using IRC `Say`; keep send status visible and preserve user text on failure. | Normal send path and failed-send feedback. | Reply send, `/me`, optimistic echo reconciliation, richer rate-limit guidance. | Fake sender success/failure tests; manual send to test channel. |
...
| R-007 | Auth | Accept Twitch username and OAuth token securely and require `chat:read`/`chat:edit` for IRC MVP. | Flags/env/config credentials; actionable missing/invalid credential errors; no secret printing. | Interactive login, refresh/secure storage, Helix token validation with scope checks. | Config/auth validation tests; redaction tests; manual invalid-token check. |
| R-008 | Config | Resolve effective config from flags > environment > config file > interactive/defaults. | File/env/flag support for username, token, channels, image mode, avatar mode, emoji mode, emote mode, animation mode. | `twi login`, setup wizard, config migration, OS keychain. | Precedence tests; `config show` redaction tests; path tests by platform. |
...
| R-011 | Diagnostics | Surface connection, send, auth, terminal capability, and degraded-mode status without destroying chat context. | Status bar/errors in TUI; secret-safe messages. | `twi doctor` for config path, scopes, Twitch reachability, terminal true color/mouse/Kitty/cache checks. | Unit tests for diagnostic output redaction; manual `doctor` later. |
