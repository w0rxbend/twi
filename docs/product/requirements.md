...
| Inline terminal images | Partial | Renderer fallback rows, fixed-width prepared cell substitution, cache boundaries, capability diagnostics, and visible-row asset event scheduling exist; default live resolver wiring and manual Kitty/Ghostty validation remain planned. |
...
Later scope includes default live image resolver wiring, image-backed avatars/emotes/emoji in validated Kitty/Ghostty-compatible terminals, startup token validation, interactive login, setup wizard, secure credential storage, activity charts, and additional transport/image protocols.
...
| R-005 | Emoji images | Preserve Unicode emoji and grapheme clusters in messages. | Native Unicode emoji fallback with grapheme-safe reveal/wrap plus provider metadata for URL-free emoji asset keys. | Decode/cache provider images and render through image pipeline by default live wiring. | Grapheme tests; golden rows with emoji; provider/cache tests; image lookup fake later. |
