...
| Login/setup | Partial | `twi setup` writes non-secret config values and can hand off to login. `twi login` runs the browser/local-callback OAuth flow or a `--dry-run` explanation, validates returned tokens, and saves them through the restrictive credential-file fallback without printing them. |
