...
| Multi-channel live IRC read/send | Partial | Reads, sends, replies, and sends `/me` actions for configured channels with environment, flat config, or saved credentials on supported platforms. Broader manual Twitch evidence remains future work. |
...
| Login/setup | Partial | `twi setup` writes non-secret config values and can hand off to login. On supported Unix and Windows builds, `twi login` runs the browser/local-callback OAuth flow or a `--dry-run` explanation, validates returned tokens, and saves them without printing them. Unix uses the restrictive credential-file fallback; Windows uses native Credential Manager. Other non-Unix builds use env/config credentials until a backend is selected. |
...
- Credentials from environment variables, config file, or saved credentials on supported platforms, with CLI overrides for channel and config path.
...
Later scope includes image-backed avatars/emotes/emoji in manually validated Kitty/Ghostty-compatible terminals, startup token validation, activity charts, native credential backend decisions for other non-Unix targets, and additional transport/image protocols.
...
| R-007 | Auth | Accept Twitch username and OAuth token securely and require `chat:read`/`chat:edit` for IRC MVP. | Env/config credentials plus saved credentials on Unix and Windows; actionable missing/invalid credential errors; doctor token validation; no secret printing; `twi login` OAuth validation; restrictive Unix credential-file persistence; Windows Credential Manager persistence; setup login handoff; refreshed-token persistence after IRC auth refresh where the credential store is supported; credential storage boundary and fakes. | Auth flags, startup token validation with scope checks, and continued deferral for other non-Unix targets until a backend is selected. | Config/auth/storage validation tests; redaction tests; manual invalid-token check. |
| R-008 | Config | Resolve effective config from CLI overrides > environment > config file > saved credentials > defaults. | Env/config support for username, token, channels, image mode, avatar mode, emoji mode, emote mode, and animation mode; CLI overrides for config path and channels; login reads client ID/secret from env/config; setup writes non-secret config values; saved credential values fill empty credential fields on supported platforms. | Auth/mode flags, explicit config-to-credential migration UX, and native backend decisions for other non-Unix targets. | Precedence tests; `config show` redaction tests; setup wizard tests; path tests by platform. |
