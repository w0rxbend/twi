...
- Partially shipped auth behavior: `twi login` runs a no-persistence browser/local-callback OAuth flow with a `--dry-run` explanation path.
- Planned behavior: setup wizard wiring, credential storage implementation using the defined boundary, and manual Kitty/Ghostty inline image validation.
