...
- Planned behavior: wiring token validation into startup, `twi login`, multi-channel live chat, persistent disk-backed asset storage, live asset metadata adapters, and inline terminal image rendering.
...
- `internal/storage` defines the context-aware asset cache boundary, an in-memory test cache, and filesystem probes used by diagnostics.
