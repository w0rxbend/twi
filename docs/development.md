...
- Planned behavior: setup wizard, secure credential storage, and manual Kitty/Ghostty inline image validation.
...
- `internal/storage` defines the context-aware asset cache boundary, an in-memory test cache, a disk-backed asset cache for metadata, public source URLs, URL-free payload identities, downloaded bytes, prepared renderer bytes, TTL/size pruning, and filesystem probes used by diagnostics.
