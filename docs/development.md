...
- `internal/storage` defines the context-aware asset cache boundary, an in-memory test cache, a disk-backed asset cache for metadata, public source URLs, URL-free payload identities, downloaded bytes, prepared renderer bytes, TTL/size pruning, filesystem probes used by diagnostics, and the credential storage boundary. Credential storage includes a redacted `CredentialRecord` DTO, test fakes, a versioned fallback JSON format, explicit token reveal helpers for file persistence, a restrictive fallback file store with exact `0700` directory/`0600` file mode requirements, atomic replacement, and symlink rejection. Unix builds use a no-follow open; non-Unix no-follow/ACL hardening remains tracked separately. No OS keychain backend is implemented yet.
